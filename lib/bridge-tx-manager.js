...
BridgeTxManager.prototype.buildLocalTx = function (txData) {
  ...
  const gasPriceNode = BlockchainInterface().inter.gasPrice.toNumber()

  const gasPriceTx = gasPriceNode > BlockchainInterface().DEFAULT_GAS ? BlockchainInterface().DEFAULT_GAS : gasPriceNode

  const rawTx = {
    nonce: bridgeUtil.intToHex(nonce),
    gasPrice: bridgeUtil.intToHex(gasPriceTx),
...
BridgeTxManager.prototype.sendActiveTx = function (txData, callback) {
  ...
  delete txData['$skip_confirmation']
