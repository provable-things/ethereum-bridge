...
BridgeCliParse.prototype.getConfiguration = function () {
  ...
  if (ops.gas) {
    ops.gas = parseInt(ops.gas)
    if (ops.gas < 21000) {
      // FIXME: errors thrown but not caught or handled...
      throw new Error('Gas amount lower than 21000 is not allowed')
    } else if (ops.gas > 5000000) {
      throw new Error('Gas amount bigger than 5000000 is not allowed')
    } else {
...
BridgeCliParse.prototype.getOptions = function () {
  return {
    ...
    'gas': {args: 1, description: 'change gas amount limit used to deploy contracts(in wei)', default: 4000000},
