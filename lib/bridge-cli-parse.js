...
BridgeCliParse.prototype.getConfiguration = function () {
  ...
  if (ops.gas) {
    ...
    // should return warnings rather than enforce these strict outdated values?
    this.cliConfiguration.defaultGas = ops.gas
    //}
  }

  if (ops.context) {
    // for Random expects one of
    // eth_ropsten3
    // eth_rinkeby
    // eth_kovan
    this.cliConfiguration.context = ops.context
  }

  if (ops['minimum-gas-price'] && Number.isInteger(parseInt(ops['minimum-gas-price']))) {
    this.cliConfiguration.minGasPrice = parseInt(ops['minimum-gas-price'])
  }

  if (ops['connector-gas-price'] && Number.isFinite(parseFloat(ops['connector-gas-price']))) {
    this.cliConfiguration.conGasPrice = ops['connector-gas-price']
  }
...
BridgeCliParse.prototype.getOptions = function () {
  return {
    ...
    'gasprice': {args: 1, description: 'Set gas price for contracts deployment (wei)', default: 20000000000},
    'context': {args: 1, description: 'Context name (internal use)', default: null},
    'minimum-gas-price': {args: 1, description: 'Minimum gas price in wei to process, queries lower than this are ignored', default: null},
    'connector-gas-price': {args: 1, description: "Set connector default gas price (gwei), which will be the default amount used for callbacks on any contract's not using oraclize_setCustomGasPrice", default: null},
    'disable-address-watcher': {description: 'disable address watcher, checking cb address bal, for 0 gas price networks such as RSK or certain private nets.'},
  }
}
