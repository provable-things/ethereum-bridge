...
const loadCompiledBin = function (pathFile) {
  return loadLocalFile(pathFile).replace(/\s/g,'')
}
...
const uint256ToHex = function (number) {
  return ethUtil.addHexPrefix(leftPad(ethUtil.stripHexPrefix(ethUtil.intToHex(number)), 64, 0))
}
