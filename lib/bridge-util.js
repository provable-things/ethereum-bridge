...
const addDSourceTxEncode = function (datasourceObj) {
  ...
  if (ethUtil.isHexPrefixed(proofIncluded) === false && parseInt(proofIncluded) !== NaN) proofIncluded = intToHex(proofIncluded)
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addDSource(string, bytes1, uint256)', [datasourceObj.name, proofIncluded, datasourceObj.units]).toString('hex'))
}
...
const standardResult = function (result) {
  ...
  if (typeof result === 'object' && typeof result.value !== 'undefined') return result.value
  else return result
}
...
module.exports = {saveFile, saveJsonFile, getEventsName, containsProof, privateToPublic, toInt, callbackTxEncode, setCbAddressTxEncode, setAddrTxEncode, intToHex, loadLocalFile, loadLocalJson, generateNewAddress, addDSourceTxEncode, setBasePriceTxEncode}
