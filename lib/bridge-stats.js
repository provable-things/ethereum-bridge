...
function BridgeStats (logger) {
  ...
  this.logger.stats('node type:', BlockchainInterface().version.node)

  var versionMethods = ['network', 'ethereum', 'whisper']
  for (var i = 0; i < versionMethods.length; i++) {
    try {
      self.logger.stats(versionMethods[i], 'version', BlockchainInterface().version[versionMethods[i]])
    } catch (e) {
      self.logger.stats(versionMethods[i], 'version is not available')
    }
  }

  this.logger.stats('available accounts:', BlockchainInterface().inter.accounts)
  this.logger.stats('coinbase:', BlockchainInterface().inter.coinbase)
  var isMining = BlockchainInterface().inter.mining
  this.logger.stats('is mining:', isMining)
...
BridgeStats.prototype.runStats = function() {
  ...
  filter.watch(function(error, result){
    ...
    if (self.latestBlock !== block.number) {
      ...
      self.logger.stats('latest block number mined', block.number)
    }
    self.latestBlock = block.number
    if (self.blockTimestamp.length % 12 === 0) {
      var blockTime = bridgeUtil.averageDelta(self.blockTimestamp)
      self.logger.stats('block time average updated to:', blockTime, 'seconds')
    }
  })
}
