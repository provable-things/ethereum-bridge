...
const setCbAddressTxEncode = function (address) {
  ...
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addCbAddress(address,bytes1)', address, '0xff').toString('hex'))
}

const addNestedSubDSTxEncode = function (ds) {
  //address = ethUtil.addHexPrefix(address)
  //if (ethUtil.isValidAddress(address) === false) throw new Error('Address provided is not valid')
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addNestedSubDS(string)', ds).toString('hex'))
}
