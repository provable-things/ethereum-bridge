...
const addCbAddressTxEncode = function (address) {
  ...
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addCbAddress(address,byte)', address, '0x01').toString('hex'))
}

const addCbAddressTypeTxEncode = function (address, type) {
  address = ethUtil.addHexPrefix(address)
  if (ethUtil.isValidAddress(address) === false) throw new Error('Address provided is not valid')
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addCbAddress(address,byte)', address, type).toString('hex'))
}

const addCbAddressProofTxEncode = function (address, type, proof) {
  address = ethUtil.addHexPrefix(address)
  if (ethUtil.isValidAddress(address) === false) throw new Error('Address provided is not valid')
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addCbAddress(address,byte,bytes)', address, type, proof).toString('hex'))
}
