...
const addCbAddressTxEncode = function (address) {
  ...
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addCbAddress(address,bytes1)', address, '0x01').toString('hex'))
}

const addCbAddressTypeTxEncode = function (address, type) {
  address = ethUtil.addHexPrefix(address)
  if (ethUtil.isValidAddress(address) === false) throw new Error('Address provided is not valid')
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addCbAddress(address,bytes1)', address, type).toString('hex'))
}

const addCbAddressProofTxEncode = function (address, type, proof) {
  address = ethUtil.addHexPrefix(address)
  if (ethUtil.isValidAddress(address) === false) throw new Error('Address provided is not valid')
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('addCbAddress(address,bytes1,bytes)', address, type, proof).toString('hex'))
}
...
const setGasPriceTxEncode = function (price) {
  price = bridgeUtil.normalizePrice(price)
  price = bridgeUtil.web3.toWei(price, 'gwei')
  return ethUtil.addHexPrefix(ethAbi.simpleEncode('setGasPrice(uint256)', price).toString('hex'))
}
...
module.exports = {
  callbackTxEncode,
  addCbAddressTxEncode,
  addCbAddressTypeTxEncode,
  addCbAddressProofTxEncode,
  setAddrTxEncode,
