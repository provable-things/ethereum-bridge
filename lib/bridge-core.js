...
const OAR_ONLY_PRIV_KEY = new Buffer('79a98ade62c92444178d73409fbce37a360b36a2483dda666d26270c8a50f5c7', 'hex')
...
function OracleInstance (config) {
  ...
  underscore.bindAll(this, 'changeOwner', 'moveAccountFunds', 'sendActiveTx', 'sendRawTx', 'addDSource', 'setBasePrice', 'checkTransaction', 'isOracleEvent', 'isValidOracleInstance', 'getCbAddresByConnector', 'getConnectorByOAR', 'getContractInstance', 'checkAccountBalance', 'deployOAR', 'deployConnector', 'sendTx', 'buildLocalTx', 'setCbAddress', 'setAddr', 'setPricing')
}
...
const getAccountNonce = function (address) {
  return web3.eth.getTransactionCount(ethUtil.addHexPrefix(address))
}

const getTempAccount = function () {
...
OracleInstance.prototype.buildLocalTx = function (txData) {
  ...
  if (typeof txData['$temporary_account'] !== 'undefined' && txData['$temporary_account'] === true) {
    broadcastAccount = OAR_ONLY_ADDRESS
    broadcastPrivateKey = OAR_ONLY_PRIV_KEY
  }
  const nonce = getAccountNonce(broadcastAccount)
  const rawTx = {
...
OracleInstance.prototype.deployOAR = function (callback) {
  ...
  if (this.deterministicOar === false || (this.deterministicOar === true && getAccountNonce(getTempAccount())) > 0) {
    ...
    async.waterfall([
      function (asyncCallback) {
        self.moveAccountFunds(OAR_ONLY_ADDRESS, 8027853000000001, asyncCallback)
      },
      function tempOarDeploy (result, asyncCallback) {
        self.sendRawTx({'from': self.account, 'data': oraclizeAddressResolverBin, 'gas': 324702, '$temporary_account': true}, function (err, contract) {
          ...
          } else asyncCallback(new Error('No contract found'), null)
        })
      }],
      function (err, result) {
        if (err) return callback(err, null)
...
module.exports = {OracleInstance, web3, ethUtil, getAccountNonce, getTempAccount}
