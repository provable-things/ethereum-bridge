...
OracleInstance.prototype.moveConnectorFunds = function (to, amount, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'gas': this.deployGas, 'to': this.connector, 'data': this.withdrawFundsTxEncode(to, amount)}, function (err, contract) {
    if (err) return callback(err, null)
    else return callback(null, true)
  })
}
...
OracleInstance.prototype.addDSource = function (connector, datasourceObj, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'to': connector, 'data': bridgeUtil.addDSourceTxEncode(datasourceObj), 'gas': this.deployGas}, function (err, contract) {
...
OracleInstance.prototype.multiAddDSource = function (connector, datasourceObj, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'to': connector, 'data': bridgeUtil.multiAddDSourceTxEncode(datasourceObj), 'gas': this.deployGas}, function (err, contract) {
    if (err) return callback(new Error(err), false)
...
OracleInstance.prototype.setBasePrice = function (connector, price, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'to': connector, 'data': bridgeUtil.setBasePriceTxEncode(price), 'gas': this.deployGas}, function (err, contract) {
...
OracleInstance.prototype.setCbAddress = function (connector, cbAddress, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'to': connector, 'data': bridgeUtil.setCbAddressTxEncode(cbAddress), 'gas': this.deployGas}, function (err, contract) {
    if (err) return callback(new Error(err), null)
...
OracleInstance.prototype.setAddr = function (oar, connector, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'to': oar, 'data': bridgeUtil.setAddrTxEncode(connector), 'gas': this.deployGas}, function (err, contract) {
    if (err) return callback(new Error(err), null)
...
OracleInstance.prototype.changeOwner = function (newOwner, oar, callback) {
  BridgeTxManager().sendRawTx({'from': this.account, 'to': oar, 'data': bridgeUtil.changeOwnerTxEncode(newOwner), 'gas': 36913, '$temporary_account': true}, function (err, contract) {
    if (err) return callback(new Error(err), null)
