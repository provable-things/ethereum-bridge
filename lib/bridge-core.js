...
OracleInstance.prototype.getCbAddressByConnector = function (connector, account) {
  ...
  const cbAddressFound = BlockchainInterface().inter.contract(this.oraclizeConnectorABI).at(oraclizeConnector).cbAddress.call({ from: account })
...
OracleInstance.prototype.getConnectorByOAR = function (oar) {
  ...
  const cbAddress = this.getCbAddressByConnector(oraclizeConnector, this.account)
...
OracleInstance.prototype.isValidOracleInstance = function (oar, account) {
  try {
    ...
    const cbAddress = this.getCbAddressByConnector(connector, account)
...
OracleInstance.prototype.setGasPriceInGwei = function (connector, price, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'to': connector, 'data': bridgeTxEnc.setGasPriceTxEncode(price), 'gas': this.deployGas}, function (err, contract) {
    if (err) return callback(new Error(err), null)
    if (contract === null) return callback(new Error('setGasPriceInGwei error'), null)
    return callback(null, {'success': true})
  })
}
...
OracleInstance.prototype.setCbAddress = function (connector, cbAddress, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'to': connector, 'data': bridgeTxEnc.addCbAddressTxEncode(cbAddress), 'gas': this.deployGas}, function (err, contract) {
    if (err) return callback(new Error(err), null)
