...
function OracleInstance (config) {
  ...
  new BlockchainInterface(config)
  this.oar = config.oar || ''
  ...
  new BridgeAccount({"mode": this.mode, "active_accounts": BlockchainInterface().inter.accounts, "key_file": this.keyFile, "account_selection": configAccount })
...
const getAccount = function (mode, configAccount, keyFile) {
  ...
  if (mode === 'active') {
    if ((BlockchainInterface().inter.accounts).indexOf(configAccount) === -1 && typeof BlockchainInterface().inter.accounts[configAccount] === 'undefined') throw new Error('Account ', configAccount, ' not found in available accounts')
    if (typeof configAccount === 'string' && ethUtil.isValidAddress(configAccount)) {
      ...
      return {'account': ethUtil.addHexPrefix(BlockchainInterface().inter.accounts[configAccount]), 'private_key': ''}
    }
...
module.exports = {OracleInstance, ethUtil}
