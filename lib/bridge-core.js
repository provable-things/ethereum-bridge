...
function OracleInstance (config) {
  ...
  underscore.bindAll(this, 'addDSource', 'setBasePrice', 'checkTransaction', 'isOracleEvent', 'isValidOracleInstance', 'getCbAddresByConnector', 'getConnectorByOAR', 'getContractInstance', 'checkAccountBalance', 'deployOAR', 'deployConnector', 'sendTx', 'buildLocalTx', 'setCbAddress', 'setAddr')
}
...
OracleInstance.prototype.addDSource = function (connector, datasourceObj, callback) {
  this.sendTx({'from': this.account, 'to': connector, 'data': bridgeUtil.addDSourceTxEncode(datasourceObj), 'gas': this.deployGas}, function (err, contract) {
    ...
    if (contract === null) return callback(new Error('addDSource error'), null)
...
OracleInstance.prototype.setCbAddress = function (connector, cbAddress, callback) {
  ...
  this.sendTx({'from': this.account, 'to': connector, 'data': bridgeUtil.setCbAddressTxEncode(cbAddress), 'gas': this.deployGas}, function (err, contract) {
    ...
    if (typeof self.onchain_config === 'undefined') return callback(null, {'success': true, 'connector': connector})
    async.each(self.onchain_config.pricing, function (thisPrice, innerCallback) {
      self.addDSource(connector, thisPrice, innerCallback)
    })
    self.setBasePrice(connector, self.onchain_config.base_price, function (err, res) {
      if (err) return callback(err, null)
      else return callback(null, {'success': true, 'connector': connector})
    })
  })
}
