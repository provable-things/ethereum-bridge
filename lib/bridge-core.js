...
OracleInstance.prototype.deployOAR = function (callback) {
  const self = this
  const oraclizeAddressResolverBin = ethUtil.addHexPrefix(bridgeUtil.loadCompiledBin(this.oarContractPath))
...
OracleInstance.prototype.addNestedSubDS = function (connector, ds, callback) {
  BridgeTxManager().sendTx({'from': this.account, 'to': connector, 'data': bridgeTxEnc.addNestedSubDSTxEncode(ds), 'gas': this.deployGas}, function (err, contract) {
    if (err) return callback(new Error(err), false)
    if (contract === null) return callback(new Error('addNestedSubDS error'), false)
    return callback(null, true)
  })
}
...
OracleInstance.prototype.setPricing = function (connector, callback) {
  ...
  self.multiAddDSource(connector, self.onchain_config.pricing, function (err, res) {
    if (err) return callback(new Error(err), null)
    // FIXME enter proper callbacks here or introduce promise?
    self.addNestedSubDS(connector, 'URL', function(){})
    self.addNestedSubDS(connector, 'WolframAlpha', function(){})
    self.addNestedSubDS(connector, 'IPFS', function(){})
    self.addNestedSubDS(connector, 'swarm', function(){})
    self.addNestedSubDS(connector, 'Blockchain', function(){})
...
OracleInstance.prototype.deployConnector = function (callback) {
  const self = this
  const oraclizeConnectorBin = ethUtil.addHexPrefix(bridgeUtil.loadCompiledBin(this.oraclizeConnectorContractPath))
