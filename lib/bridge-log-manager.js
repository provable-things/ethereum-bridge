...
var logsContainer = [], cliConfiguration

function BridgeLogManager (cliConfiguration) {
  this.contractInstance = OracleInstance().getContractInstance()
  setCliConfiguration(Object.assign({}, cliConfiguration))
}
...
const parseLog = function (err, log) {
  ...
  if (logArgs['gasPrice']) {
    logObj['parsed_log'].gasPrice = logArgs['gasPrice'].toNumber()
    if (cliConfiguration.minGasPrice && logObj['parsed_log'].gasPrice !== 0 && cliConfiguration.minGasPrice > logObj['parsed_log'].gasPrice) {
      return emitsNewLog('Gas price requested below set minimum', null)
    }
  }
  ...
  try {
    ...
    if (cliConfiguration.dev) {
      ...
      console.error(`There was an issue contacting the node...
Node returned error message: ${e.message}
If key not found, the blockhash just seen is gone, indicating a re-org...
if running this on ganache or a dev chain, re-run bridge with --dev flag to suppress this error`)
      return emitsNewLog('Error trying to get Block, node returned: ' + e.message, null)
    }
  }
...
const setCliConfiguration = function (conf) {
  cliConfiguration = conf
}
