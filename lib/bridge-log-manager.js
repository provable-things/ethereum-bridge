...
const emitsNewLog = function (err, log) {
  if (err) LogEmitter.emit('log-err', err)
    ...
    try {
      ...
      if (latestEventMyid.length > 0 &&
       latestEventMyid.contract_myid === contractMyid &&
       latestEventMyid.blockHash === log.blockHash &&
       (Date.now() - latestEventMyid.timestamp) < 40000) return
      latestEventMyid.contract_myid = contractMyid
      latestEventMyid.blockHash = log.blockHash
      latestEventMyid.timestamp = Date.now()
      LogEmitter.emit('new-log', log)
    } catch (e) {
      return LogEmitter.emit('log-err', e)
    }
  }
}
...
const parseLog = function (err, log) {
  ...
  if (log['event'] === 'Log1') {
    ...
    else logObj['parsed_log'].formula = cbor.decodeAllSync(Buffer.from(logArgs['args'].substr(2), 'hex'))[0]
  }
