...
const removeAllLogs = function (e, type) {
  ...
  if (typeof type !== 'undefined' && typeof type === 'object' && type.clean === true) console.log('\nPlease wait...')
  asyncLoop(logsContainer, function (listening, next) {
    if (typeof listening !== 'undefined') listening.stopWatching()
    next(null)
  }, function (err) {
    if (err) console.error(err)
    if (typeof type !== 'undefined' && typeof type === 'object' && type.clean === true) console.log('To load this instance later use --instance latest')
    console.log('Exiting...')
    process.exit()
  })
}
